...
import { formatTransactionError, getInvoiceIdsByBuyer, getInvoice, type OnChainInvoice } from '@/lib/accordpay';
...
export default function SentInvoicesPage() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      setInvoices([]);
      try {
        const ids = await getInvoiceIdsByBuyer(publicClient, CONTRACT, address);
        ...
        if (!cancelled) setError(`Could not load sent invoices. ${formatTransactionError(e)}`);
      } finally {
  ...
  if (status === 'disconnected' || status === 'connecting' || status === 'no_wallet') {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        <p className="text-slate-500">Connect your wallet to view sent invoices.</p>
      </div>
    );
  }
  ...
  if (!CONTRACT) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        <p className="text-slate-500">AccordPay contract not yet deployed. Set <code className="bg-slate-100 px-1 rounded">NEXT_PUBLIC_ACCORDPAY_ADDRESS</code>.</p>
      </div>
    );
  }
  ...
  return (
    <div className="page-shell space-y-6 py-8">
      <InvoiceSectionHeader current="sent" />
      ...
      <div>
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">No sent invoices found.</div>
        ) : (
          <InvoiceManager invoices={invoices} role="buyer" />
        )}
      </div>
    </div>
  );
